...
def _list_ref_wavs(ref_base_dir, lang_tgt, speaker_tgt):
    """Find reference wavs for a target speaker.

    Tries <ref_base_dir>/<lang_tgt>/<speaker_tgt>/ first, then
    <ref_base_dir>/<speaker_tgt>/ as a fallback.
    """
    candidates = [
        os.path.join(ref_base_dir, lang_tgt, speaker_tgt),
        os.path.join(ref_base_dir, speaker_tgt),
    ]
    for d in candidates:
        if os.path.isdir(d):
    ...
    raise FileNotFoundError(
        f"No reference wavs found for speaker '{speaker_tgt}' (lang '{lang_tgt}'). "
        f"Looked in: {candidates}"
    )


def compute_target_embedding_online(
    net_g, ref_base_dir, lang_tgt, speaker_tgt,
    sampling_rate, num_ref_wavs, cache,
):
    """Compute the target speaker embedding on the fly with the SAME encoder
    used during training (net_g.get_spk_emb -> ECAPA2). Averages over up to
    `num_ref_wavs` reference utterances for a stable speaker embedding.

    Returns a tensor of shape (1, gin_channels, 1) on cuda, ready to be passed
    as g_tgt to voice_conversion.
    """
    key = (lang_tgt, speaker_tgt)
    if key in cache:
        return cache[key]

    wavs = _list_ref_wavs(ref_base_dir, lang_tgt, speaker_tgt)
    if num_ref_wavs > 0:
        wavs = wavs[:num_ref_wavs]

    embs = []
    for wav_path in wavs:
        # Load at the model sampling rate to match training exactly:
        # during training get_spk_emb receives the audio at hps.data.sampling_rate.
        ref_wav, _ = librosa.load(wav_path, sr=sampling_rate)
        ref_wav = torch.from_numpy(ref_wav).float().unsqueeze(0).cuda()  # (1, T)
        g = net_g.get_spk_emb(y=ref_wav)  # (1, gin, 1)
        embs.append(g.squeeze(0).squeeze(-1))  # (gin,)

    g_tgt = torch.stack(embs, dim=0).mean(dim=0)  # (gin,)
    g_tgt = g_tgt.view(1, -1, 1).cuda()  # (1, gin, 1)
...
def main():
    ...
    parser.add_argument("--spk-mode", type=str, default="online",
                        choices=["online", "precomputed"],
                        help="online: compute target embedding from reference wavs "
                             "using the trained ECAPA2 encoder (consistent with training). "
                             "precomputed: load <spk_emb_base_dir>/<lang>/<speaker>.pt")
    parser.add_argument("--spk-ref-base-dir", type=str, default=None,
                        help="[online] dir containing <lang>/<speaker>/*.wav reference wavs "
                             "(e.g. dataset_custom/audio)")
    parser.add_argument("--num-ref-wavs", type=int, default=10,
                        help="[online] number of reference wavs to average per target speaker "
                             "(<=0 means use all)")
    parser.add_argument("--spk-emb-base-dir", type=str, default=None,
                        help="[precomputed] dir with <lang>/<speaker>.pt embeddings")
    ...
    if args.spk_mode == "online":
        if not args.spk_ref_base_dir:
            parser.error("--spk-ref-base-dir is required when --spk-mode=online")
    else:
        if not args.spk_emb_base_dir:
            parser.error("--spk-emb-base-dir is required when --spk-mode=precomputed")

    spk_emb_cache = {}

    # Only load the (GitHub-downloaded) Silero VAD when actually requested.
    vad_model_and_utils = None
    if args.use_vad:
    ...
    with torch.no_grad():
        for src, lang_src, speaker_src, speaker_tgt, lang_tgt in tqdm(zip(srcs, lang_srcs, speaker_srcs, speaker_tgts, lang_tgts), desc="Synthesizing", total=len(srcs)):
            ...
            if args.spk_mode == "online":
                # Compute with the SAME encoder used at training (ECAPA2),
                # so train/inference are consistent. Cached per speaker.
                g_tgt = compute_target_embedding_online(
                    net_g,
                    args.spk_ref_base_dir,
                    lang_tgt,
                    speaker_tgt,
                ...
                if g_tgt.dim() == 1:          # (gin,)      -> (1, gin, 1)
                    g_tgt = g_tgt.view(1, -1, 1)
                elif g_tgt.dim() == 2:        # (1, gin)    -> (1, gin, 1)
